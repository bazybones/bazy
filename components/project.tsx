    ...
    >
      <section className="bg-gray-100 max-w-[42rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem] hover:bg-gray-200 transition sm:group-even:pl-8">
        <div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[18rem]">
          ...
          <p className="mt-2 text-left leading-relaxed text-gray-700 ">
            {description}
          </p>
          <ul className="flex flex-wrap mt-4 gap-2 sm:mt-auto">
            {icons.map((item, index) => (
              <li
                className="flex items-center bg-white/[0.7] px-3 py-1 text-[0.7rem] uppercase font-bold tracking-wider text-teal-300 rounded-full "
                key={index}
              >
                <span className="mr-2 flex justify-center items-center w-6 h-6">
                  {item.icon}
                </span>
